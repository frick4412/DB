...
v8.2
	10/09/2018:	Changed constructor parameters to array
				...
				Changed $this->connection to local variable
...
class DB {
    ...
    protected $_dataConnParams;
    protected $_authConnParams;
	protected $_displayerrors;
	protected $_dcnn;
	protected $_acnn;

    /*  Constructor
     *  1 parameter: associative array(
	 *		"dataConn" as ini file reference, 
	 * 		"authConn" as ini file reference,
	 * 		"displayererrors" as true/false,
	 *		"skipConns" as true/false (for AD Auth, etc.)
	 *	  )
	 *	
     */
    public function __construct($params) {
		...
		pwd = mypassword
		*/
		$skipConnections = isset($params["skipConns"]) ? $params["skipConns"] : false ;
		if(!$skipConnections) {
			// Data Connection
			$this->_dataConnParams = parse_ini_file($params["dataConn"],true);
			$this->_dcnn = $this->_getConn($this->_dataConnParams);
			// Auth Connection
			if(isset($params["authConn"])) {
				$this->_authConnParams = parse_ini_file($params["authConn"],true);			
			} else {
				$this->_acnn = $this->_dcnn;
				echo $this->_acnn;
				echo "\n\n";
			}			
		}
		// Display Errors
		$this->_displayerrors = $params["displayerrors"];
    }
    ...
    public function query($q) {  
        ...
        try
        {
        $stmt = $this->_dcnn->prepare($q["sql"]);  // Use lazy loading getter
 ...
 */
	...
	private function updatePwHash($userid, $pwhash) {
		...
		$stmt = $this->_acnn->prepare($qry);  // Use lazy loading getter
	...
	public function validateJwt($jwt, $appname, $server) {
		...
		if(isset($jwtArray[1])) {
			...
			return array($result, $isValidSignature, $isValidNbf, $isValidExp, $isValidSub, $isValidIss
							, $algo, $jwt, $jwtArray, $encodedClaims, $pwhash, $clientSig, $serverSig, $aud->username, $claims["sub"]);
		} else {
			return array(0);  // fail/invalid
		}
	}
	
	private function getPasswordHash($username, $appname) {   // <---------------------- change to private
...
// ----------------------------------------------------------------------------------
    ...
    protected function _getConn($cp) {
		// Lazy load connection
		$dsn = "";
		$cnn = null;
		if($cp['db']['drvr'] == 'mssql') {
		  $dsn = "dblib:version:7.3;charset=UTF-8;host=".$cp['db']['host'].";dbname=".$cp['db']['db'].";"; // working mssql	
		  try {
			$cnn = new PDO($dsn, $cp['db']['usr'], $cp['db']['pwd']);
			$cnn -> setAttribute(PDO::ATTR_TIMEOUT, 300);  // new in v5.3
			$cnn -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$cmd = $cnn->prepare('SET ANSI_WARNINGS ON');
			$cmd->execute();
			$cmd = $cnn->prepare('SET ANSI_NULLS ON');
			$cmd->execute();
		  }
		  catch(PDOException $e) {
		...
		if($cp['db']['drvr'] == 'sqlsrv') {
		  $dsn = "sqlsrv:Server=".$cp['db']['host'].";Database=".$cp['db']['db'];
		  try {
			$cnn = new PDO($dsn, $cp['db']['usr'], $cp['db']['pwd']);
			$cnn -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
			$cnn -> setAttribute(PDO::SQLSRV_ATTR_QUERY_TIMEOUT, 300);  // added 8/24/2016 [SC]
		  }
		  catch(PDOException $e) {
			print "Connection to database failed!".$e;
			die();
		...
		if($cp['db']['drvr'] == 'mysql') {  // new in v5.3 (from DB4)
		  $dsn = "mysql:host=".$cp['db']['host'].";dbname=".$cp['db']['db'];
		  try {
			$cnn = new PDO($dsn, $cp['db']['usr'], $cp['db']['pwd']);
			$cnn -> setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
